...
class Clip < ApplicationRecord
  ...
  def self.search user_id
    Clip.includes([:boards, { restaurant: [:restaurant_pictures, :category, :station] }])
      .where(user_id: user_id).order("created_at DESC").map { |clip| clip.merge_restaurant_and_boards }
  end
  ...
  def create_boards_and_clip_categories user_id, existing_board_ids, new_board_names
    if new_board_names.present?
      board_ids =
        existing_board_ids +
        Board.import(new_board_names.map { |b_name| Board.new user_id: user_id, name: b_name }).ids
    else
      board_ids = existing_board_ids
    end
